  ...
  return <div className="modal-shield settings-shield" onPointerDown={onClose}>
    <section className="site-settings classic-dialog" onPointerDown={stop}>
      ...
      <div className="settings-body">
        ...
        {tab === 'cursors' && <div className="settings-section">{CURSOR_STATES.map((state) => <section className="settings-cursor" key={state.key}><strong>{state.emoji} {state.label}</strong><AssetField label="图片" value={draft.cursors[state.key].url} onChange={(url) => update({ cursors: { ...draft.cursors, [state.key]: { ...draft.cursors[state.key], url } } })} /><label>Hotspot X {draft.cursors[state.key].hotspotX}<input type="range" min="0" max="31" value={draft.cursors[state.key].hotspotX} onChange={(event) => update({ cursors: { ...draft.cursors, [state.key]: { ...draft.cursors[state.key], hotspotX: Number(event.target.value) } } })} /></label><label>Hotspot Y {draft.cursors[state.key].hotspotY}<input type="range" min="0" max="31" value={draft.cursors[state.key].hotspotY} onChange={(event) => update({ cursors: { ...draft.cursors, [state.key]: { ...draft.cursors[state.key], hotspotY: Number(event.target.value) } } })} /></label></section>)}</div>}
