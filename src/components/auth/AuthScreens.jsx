  ...
  return (
    <div className="auth-overlay"><div className="auth-card win98-window">
      <div className="auth-header">
        <span className="auth-icon">🖥️</span><h1>deskof.me</h1><p>欢迎来到你的数字桌面！</p>
      </div>
      <div className="auth-body">
        <p className="auth-desc">你是第一个访问者，将被设为<strong>唯一管理员</strong>。<br/>设置密码后即可自由编辑桌面上的一切。</p>
...
export function LoginScreen({ onLogin, onCancel, onContinueAsGuest }) {
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
