...
**隐藏登录入口**：
- **主入口**：快速连击任务栏右下角时钟 **5 次**（3 秒内完成），弹出登录窗口
- **兜底入口**：URL 参数 `?admin`（如 `deskof.me/?admin`）
- 访客在 Start 菜单、右键菜单等 UI 中**看不到任何登录入口**
- 目的：保持桌面界面干净，不被「登录」按钮破坏沉浸感

**注册流程**：
- 如果当前浏览器没有管理员密码记录，登录窗口底部显示「🆕 首次设置管理员密码」按钮
- 点击后进入密码设置界面（两次确认）
- 设置成功后自动登录，记住会话
...
- 图标支持两种尺寸：普通和大号（视觉差异明显，大号约为普通 1.7 倍）
- 图标 hover 时微放大，不抖动
...
- 文件夹：较宽（~720×500），适合网格布局
- 图片：接近正方形（~700×520）
- 视频：16:9 横向（~720×500）
- Markdown：适合阅读的宽度（~620×500）
- 链接：明显更小的卡片尺寸（~380×200）

**窗口自定义**：
- 管理员可为每种文件类型的窗口设置背景装饰图
- 窗口标题栏、边框、最大化/最小化/关闭按钮均可替换为自定义图片

### 3.5 任务栏
- 底部固定横条
- 左侧 Start 按钮（点击弹出开始菜单）
- 中间显示已打开窗口的任务按钮（点击切换/还原）
- 右侧显示管理员标识（编辑模式时显示 ✎）+ 时钟
- 任务栏背景、Start 图标等可替换
- **时钟区域连击 5 次触发管理员登录**
...
- Start 按钮弹出，Win98 风格
- 左侧竖排品牌标识
- 菜单项可由管理员动态配置：添加/删除/修改标签、图标、超链接
- 支持分隔线
- 支持仅管理员可见的菜单项
- **不包含管理员登录入口**（已移至隐藏触发方式）

### 3.7 右键菜单
- 桌面空白处右键：进入编辑模式 / 新建文件 / 添加装饰 / 站点设置（均为管理员可见）
...
**发布流程**：
- 管理员在站点设置中填入 GitHub Personal Access Token（`repo` 权限）
- 点击「发布到 GitHub」→ 自动将当前所有内容（桌面项目、配置、装饰）commit 到 GitHub 仓库源文件
- GitHub Pages 自动重建 → 约 1 分钟后所有访客可见
- 版本号机制：发布时递增 `dataVersion`，访客浏览器检测到新版本自动清空旧 localStorage 缓存
...
### 3.11 自定义光标系统
- 全局隐藏系统光标（`cursor: none !important`），使用自定义光标
- 根据鼠标下方元素类型自动切换光标样式
...
### 4.5 管理员登录触发
- 快速连续点击任务栏右下角时钟 5 次（3 秒内）
- 成功后弹出管理员登录窗口
- 备选：URL 添加 `?admin` 参数直接触发
...
interface SiteConfig {
  ...
  cursorGrabbing: string | null;
  cursorMove: string | null;
  cursorResize: string | null;
...
interface StartMenuItem {
  ...
  label: string;
  icon: string;
  action: 'toggleEdit' | 'openSettings' | 'logout' | 'link';
  url?: string;
  adminOnly?: boolean;
  type?: 'separator';
  // 双态支持（如编辑模式切换时的文字和图标变化）
  editLabel?: string;
  editIcon?: string;
}
```
> **注意**：`action` 字段不再包含 `'showLogin'`。管理员登录通过隐藏触发（时钟连击 / URL 参数），不应出现在菜单中。
...
### 9.1 首次访问（管理员）
1. 打开网站 → 直接看到桌面（默认内容）
2. 快速连击任务栏右下角时钟 **5 次**（3 秒内）→ 弹出登录窗口
3. 点击「🆕 首次设置管理员密码」→ 设置密码
4. 成功 → 自动进入管理状态
5. Start →「进入编辑模式」→ 开始编辑
...
2. 可以双击浏览所有内容
3. 无法编辑
4. Start 菜单、右键菜单中均无登录入口

### 9.3 日常编辑
1. 管理员连击时钟 5 次 → 弹出登录窗口 → 输入密码
2. Start →「进入编辑模式」
3. 添加/修改桌面内容（实时保存到 localStorage）
4. 满意后 → 设置面板 → 发布 → 访客可见

### 9.4 忘记登录方式
1. 在网址后加 `?admin` → 直接弹出登录窗口
...
- [ ] 访客打开即见桌面，无登录阻断
- [ ] Start 菜单和右键菜单中无管理员登录入口
- [ ] 快速点击时钟 5 次弹出管理员登录窗口
- [ ] `?admin` URL 参数可触发管理员登录
- [ ] 所有文件类型可双击打开对应窗口
- [ ] 窗口可拖拽、缩放、全屏、最小化、关闭
- [ ] 管理员可添加/删除/重命名桌面项目
- [ ] 管理员可拖拽文件移入/移出文件夹
